...
- Architecture and technology decisions for the AWS migration (Fargate vs EC2/EKS/Lambda, CloudFront for HTTPS without a domain, no NAT gateway, CloudWatch instead of Prometheus/Grafana, removing Jenkins) and the trade-offs behind them.
...
**Say it honestly:**
- The Django application code was written with substantial AI assistance (Claude). Do not claim you hand-coded the application.
...
- **Application Load Balancer** - health checks, zero-downtime rollouts; reachable only from CloudFront
- **Amazon CloudFront** - HTTPS, HTTP->HTTPS redirect, caching, S3 origin with Origin Access Control
- **Amazon RDS for PostgreSQL 17** - private subnets, encryption at rest, TLS enforced
- **Amazon S3** - private bucket for user uploads; separate encrypted bucket for Terraform state
...
Designed, validated, not deployable in Academy: least-privilege IAM task/execution roles and GitHub OIDC federation (the lab forbids creating IAM roles and identity providers; `LabRole` and session credentials are used instead).
...
- Application hardening: nonce-based Content-Security-Policy (removed runtime third-party script), HSTS and security headers, secure cookies, brute-force lockout, per-user/IP rate limiting, upload validation.
...
- Network: VPC with public/private subnets in 2 AZs; CloudFront -> ALB -> task -> database, each hop allowed only from the previous one (managed prefix list + secret origin header + security-group references); database without internet route; no NAT gateway (cost).
- Edge: HTTPS without a domain through CloudFront; static and media caching; uploads served from S3 via OAC.
...
- The full request path and the controls on each hop, including why the CloudFront->ALB hop is HTTP and how `CloudFront-Forwarded-Proto` solves the resulting redirect loop.
...
**EPIConnect - Campus platform modernised to AWS with DevSecOps** *(Django, Docker, AWS ECS Fargate, RDS, CloudFront, Terraform, GitHub Actions)*
- Migrated a Django campus platform from a single Azure VM to AWS: containerised (non-root, read-only), ECS Fargate behind ALB + CloudFront, RDS PostgreSQL in private subnets, S3 for uploads, Secrets Manager; infrastructure in Terraform.
