...
The application code was written with substantial help from an AI assistant (Claude). This repository does **not** claim that Rayen hand-wrote every line of the Django application.
...
- **Application secrets:** generated by Terraform (`random_password`), stored in Secrets Manager, injected by the ECS agent. They are in the Terraform state, which lives in a private, encrypted, versioned, TLS-only S3 bucket (`scripts/bootstrap-tfstate.sh`).
